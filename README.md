...
🔒 **Pre-Commit Hooks (Only first time)**

To enable automated linting and formatting checks before each commit, install the pre-commit hooks using:

```bash
make bootstrap
```

This installs the repository’s .pre-commit-config.yaml hooks into your .git/hooks directory.
...
| `make bootstrap` | Install pre-commit hooks on host .git/hooks/.|
...
| `make poetry-add PACKAGE=<package>` | Install a new regular dependency inside container and update `pyproject.toml` |
| `make poetry-add-dev PACKAGE=<package>` | Install a new development dependency (`-D`) inside container and update `pyproject.toml` |
